...
export class RepoRunner implements HealingRunner, CodeRunner {
  ...
  async generate(opts: {
    ...
    return this.withRepo(parsed.owner, parsed.repo, async () => {
      ...
      // markdown フェンス除去してからパース
      const cleaned = raw.replace(/```json\s*/gi, "").replace(/```\s*/g, "").trim();
      let patches: Patch[] = [];
      let error: string | undefined;
      try {
        const parsedJson = JSON.parse(cleaned);
        if (Array.isArray(parsedJson.patches)) patches = parsedJson.patches as Patch[];
        else if (Array.isArray(parsedJson)) patches = parsedJson as Patch[];
        else error = "AI の応答に patches 配列が含まれていませんでした。";
      } catch {
        error = `AI 応答の JSON パースに失敗しました: ${cleaned.slice(0, 200)}`;
      }
