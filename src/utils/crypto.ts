 ...
 * @param secretKey OURO_ENCRYPTION_KEY（Workers では process.env が無いため明示渡し）
 */
export async function encrypt(text: string, secretKey = DEFAULT_SECRET): Promise<string> {
  if (!text) return "";
 ...
 */
export async function decrypt(encryptedBase64: string, secretKey = DEFAULT_SECRET): Promise<string> {
