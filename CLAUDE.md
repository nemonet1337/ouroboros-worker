...
- Workers にはファイルシステム・git・コンパイラが無いため、重い処理は外部 runner (`RUNNER_URL`) へ HTTP 委譲
...
src/                   Worker ソース（全ビジネスロジック + CF アダプター）
  ...
  index.ts             Worker エントリーポイント（fetch / queue / scheduled）
...
Workflows (healing.ts)
  1. scan     → DispatchRunner → 外部 runner /internal/scan
  2. analyze  → AIAnalyzer → Workers AI
  3. fix      → DispatchRunner → 外部 runner /internal/heal → VCS.createPR()
...
RUNNER_SHARED_SECRET     外部 runner 認証（RUNNER_URL 設定時のみ必要）
